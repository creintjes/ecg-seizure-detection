#!/usr/bin/env python3
"""
Batch evaluation of TimeVQVAE-AD results from a directory of pickle files (PARALLEL VERSION).
Processes multiple joint_anomaly_score.pkl files and generates:
- CSV with all metrics
- Individual plots per file
- Summary report (TXT)

This is a parallelized version of batch_evaluate_timevqvae.py using multiprocessing.
"""
...
def process_file_worker(args: Tuple) -> Tuple[Optional[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    Args:
        args: Tuple of (pkl_path, scale, clustering_strategy, output_dir, save_plots)

    Returns:
        Tuple of (file_metrics_dict, seizure_detections_list)
    """
    pkl_path, scale, clustering_strategy, output_dir, save_plots = args
    return evaluate_single_file(pkl_path, scale, clustering_strategy, output_dir, save_plots)
...
def generate_summary_report(results_df: pd.DataFrame, output_path: Path):
    ...
    with open(output_path, 'w') as f:
        f.write("="*80 + "\n")
        f.write("TIMEVQVAE-AD BATCH EVALUATION SUMMARY REPORT (PARALLEL)\n")
        f.write("="*80 + "\n")
...
def main():
    parser = argparse.ArgumentParser(description='Batch evaluate TimeVQVAE-AD results (PARALLEL VERSION)')
    ...
    parser.add_argument('--workers', type=int, default=7, help='Number of parallel workers (default: 4)')
    ...
    print("="*80)
    print("TIMEVQVAE-AD BATCH EVALUATION (PARALLEL)")
    print("="*80)
    ...
    worker_args = [
        (pkl_file, args.scale, args.clustering, output_dir, not args.no_plots)
