...
def load_args():
    ...
    parser.add_argument('--rolling_window_stride_rate', default=0.5, type=float)
...
if __name__ == '__main__':
    ...
    if args.dataset_ind[0] == 'all':
        subjects = [f"{idx:03d}" for idx in range(97, 126)]
    else:
    ...
    for sub in subjects:
        ...
        for wsr in args.latent_window_size_rates:
            result_fname = get_root_dir().joinpath('evaluation', 'results', f'{sub}-anomaly_score-latent_window_size_rate_{wsr}.pkl')
