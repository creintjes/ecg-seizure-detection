#!/usr/bin/env python3
"""
Stage 2 Grid Search Automation Script

This script automates grid search for stage 2 model training by:
1. Iterating through different parameter combinations
2. Updating the config.yaml file for each combination
3. Running the stage2.py training script
4. Cleaning up saved models between runs
5. Logging all configurations and results via WandB

Usage:
    python test_stage2_grid_search.py [--config path/to/config.yaml] [--dry-run]
"""
...
def load_args():
    ...
    parser.add_argument('--gpu_device_ind', nargs='+', default=[0], type=int,
                       help="GPU device indices to use for training")
...
def main():
    ...
    # Define parameter grid for stage 2 MaskGIT model
    # Focus on most impactful MaskGIT architecture parameters
    param_grid = {
        "MaskGIT.prior_model.depth": [8, 10, 12],
        "MaskGIT.prior_model.heads": [6, 8, 12],
        "MaskGIT.prior_model.attn_dim_head": [32, 48, 64]
    }
    ...
    print(f"  MaskGIT depth: {param_grid['MaskGIT.prior_model.depth']}")
    print(f"  MaskGIT attention heads: {param_grid['MaskGIT.prior_model.heads']}")
    print(f"  MaskGIT attention dim per head: {param_grid['MaskGIT.prior_model.attn_dim_head']}")
    print(f"  Batch size: stage2 batch size from config")
    print(f"  Max hours per run: stage2 max hours from config")
    print(f"\nTotal combinations to test: {len(combinations)}")
    print(f"Estimated total time: {len(combinations) * 12} hours (12 hours per combination)")
    ...
    try:
        for i, combination in enumerate(combinations):
            ...
            success = run_stage2_training(args.gpu_device_ind, args.dataset_ind)
