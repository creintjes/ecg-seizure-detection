...
# -*- coding: utf-8 -*-
"""
Responder Detection with Elgendi R-Peak Detection Method (All Seizures Criteria)
=================================================================================

This script classifies patients as responders based on a stricter criterion:
A patient is a RESPONDER if and only if ALL their seizures show a maximum
HR change > 50 BPM.

Differences from find_responder_elgendi.py:
- Original: Patient is responder if MEAN(max HR changes) > 50 BPM
- This script: Patient is responder if ALL max HR changes > 50 BPM
...
# Output files
OUTPUT_TXT = "responders_and_non_responders_elgendi_all_seizures.txt"
OUTPUT_CSV = "patient_responder_summary_elgendi_all_seizures.csv"
...
def main():
    ...
    print("=" * 70)
    print("Responder Detection with Elgendi (All Seizures Criteria)")
    print("=" * 70)
    print()
    print("Classification criterion:")
    print("  RESPONDER: ALL seizures have max HR change > 50 BPM")
    print("  NON-RESPONDER: At least one seizure has max HR change <= 50 BPM")
    print()
    ...
    # Responder: ALL seizures must have max HR change > threshold
    # Non-responder: At least one seizure has max HR change <= threshold
    ...
    for patient_id, maxima in patient_seizure_maxima.items():
        ...
        # Check if ALL seizures exceed threshold
        all_above_threshold = all(m > BPM_THRESHOLD for m in valid)
    ...
    with open(OUTPUT_TXT, "w", encoding="utf-8") as f:
        ...
        f.write("# Classification: ALL seizures must have max HR change > threshold\n")
    ...
    with open(OUTPUT_CSV, "w", newline="", encoding="utf-8") as csvf:
        ...
        writer.writerow(["subject_id", "n_seizures", "mean_max_bpm", "std_max_bpm",
                         "min_max_bpm", "max_max_bpm", "all_above_threshold", "label"])
        for pid, maxima in sorted(patient_seizure_maxima.items()):
            ...
            if n > 0:
                ...
                all_above = all(m > BPM_THRESHOLD for m in valid)
                label = "responder" if all_above else "non_responder"
            else:
                mean_m = std_m = min_m = max_m = float("nan")
                all_above = False
                label = "unknown"
            writer.writerow([pid, n, mean_m, std_m, min_m, max_m, all_above, label])

    # Print summary
    print("\n" + "=" * 70)
    print("RESULTS SUMMARY (ALL SEIZURES CRITERIA)")
    print("=" * 70)
    print(f"Responders:     {len(responders):3d} (ALL seizures > {BPM_THRESHOLD} BPM)")
    print(f"Non-responders: {len(non_responders):3d} (At least one seizure <= {BPM_THRESHOLD} BPM)")
