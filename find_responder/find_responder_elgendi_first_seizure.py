...
# -*- coding: utf-8 -*-
"""
Responder Detection with Elgendi R-Peak Detection - First Seizure Only
=====================================================================

This script classifies responders based on ONLY the first recorded seizure
per patient, following the methodology from the paper:

"We considered responders to be the patients in whom the first recorded
seizure had an ictal heart rate change of >50 beats/min during a 100 R-R
interval period."

Key differences from find_responder_elgendi.py:
- Only processes the FIRST seizure per patient (chronologically earliest)
- Uses Elgendi R-peak detection method
...
# Output files
OUTPUT_TXT = "responders_and_non_responders_elgendi_first_seizure.txt"
OUTPUT_CSV = "patient_responder_summary_elgendi_first_seizure.csv"
PLOT_DIR = "responder_plots_first_seizure"
...
def plot_seizure_with_rpeaks(subject_id: str, run_id: str, seizure_info: Dict,
    ...
    """
    Create a plot showing the first seizure with detected R-peaks.

    Args:
        subject_id: Subject identifier
        run_id: Run identifier
        seizure_info: Dictionary with seizure information (start_time, duration, max_hr_change)
    ...
    try:
        ...
        ax1.set_title(f'First Seizure - {patient_id} ({subject_id}_{run_id})\n'
        ...
        filename = f"responder_{patient_id}_{subject_id}_{run_id}_first_seizure.png"
...
def process_recording(subject_id: str, run_id: str) -> Optional[Dict]:
    ...
    - subject_id
    - run_id
    - seizures: list of dicts with {start_time, duration, max_hr_change}
...
def main():
    ...
    print("=" * 70)
    print("Responder Detection - FIRST SEIZURE ONLY")
    print("Using Elgendi R-Peak Detection")
    print("=" * 70)
    print()
    print("Classification criterion:")
    print("  \"We considered responders to be the patients in whom the")
    print("   first recorded seizure had an ictal heart rate change of")
    print("   >50 beats/min during a 100 R-R interval period.\"")
    print()
    print(f"Parameters:")
    ...
    with ProcessPoolExecutor(max_workers=workers, initializer=init_worker) as executor:
        ...
        for idx, future in enumerate(as_completed(future_to_rec), 1):
            ...
            if result is not None:
                ...
                processed_count += 1
                print(f"[{idx}/{total}] {subject_id} {run_id} -> OK ({len(result['seizures'])} seizures)")
            else:
                failed_count += 1
    ...
    # Group by patient and find FIRST seizure
    ...
    for rec_result in recording_results:
        ...
        if patient_id not in patient_first_seizure:
            ...
            existing_start = patient_first_seizure[patient_id]['seizure']['start_time']
            new_start = first_seizure_this_recording['start_time']

            # Note: We can't directly compare across recordings without global timestamp
            # So we take the first seizure from the alphabetically first recording
            # (This is a limitation - ideally we'd have absolute timestamps)
    ...
    with open(OUTPUT_TXT, "w", encoding="utf-8") as f:
        f.write("# Responders and non-responders - FIRST SEIZURE ONLY\n")
        f.write("# Using Elgendi R-peak detection\n")
        f.write("# Classification: First recorded seizure with HR change >50 bpm (100 RR intervals)\n")
    ...
    with open(OUTPUT_CSV, "w", newline="", encoding="utf-8") as csvf:
        ...
        writer.writerow(["subject_id", "recording", "seizure_start_time",
    ...
    # Print summary
    print("\n" + "=" * 70)
    print("RESULTS SUMMARY - FIRST SEIZURE ONLY")
    print("=" * 70)
