#!/usr/bin/env python3
"""
Madrid File-Level Threshold Trade-off Analysis
...
class MadridThresholdTradeoffAnalyzer:
    def __init__(self, results_dir: str, output_dir: str = None):
        """
        Initialize the threshold trade-off analyzer.
    ...
    def run_threshold_analysis(self, thresholds: List[float]) -> List[Dict[str, Any]]:
        ...
        print(f"Running analysis for {len(thresholds)} thresholds...")
    ...
    def create_tradeoff_plot(self, results: List[Dict[str, Any]], output_filename: str):
        ...
        ax1.set_title('File-Level Sensitivity vs False Alarms per Hour\n(Madrid File-Level Metrics)')
        ...
        ax2.set_title('Seizure-Level Sensitivity vs False Alarms per Hour\n(Madrid File-Level Metrics)')
        ...
        ax3.set_title('Combined Sensitivity Comparison\n(Madrid File-Level Metrics)')
        ax3.legend()
        ...
        ax4.set_title('Total Detections vs Threshold\n(Madrid File-Level Metrics)')
        ...
        print(f"Trade-off plot (PDF) saved to: {pdf_path}")
    ...
    def save_results(self, results: List[Dict[str, Any]], output_filename: str):
        ...
        with open(json_path, 'w') as f:
            json.dump({
                'analysis_metadata': {
                    'analysis_type': 'threshold_tradeoff_file_level',
                    'timestamp': timestamp,
        ...
        with open(csv_path, 'w') as f:
            f.write("threshold,sensitivity_file_level,sensitivity_seizure_level,false_alarms_per_hour,total_detections,total_seizures,total_detected_seizures,files_processed,files_with_seizures,seizure_files_detected,total_duration_hours\n")
            for r in results:
                f.write(f"{r['threshold']},{r['sensitivity_file_level']},{r['sensitivity_seizure_level']},{r['false_alarms_per_hour']},{r['total_detections']},{r['total_seizures']},{r['total_detected_seizures']},{r['files_processed']},{r['files_with_seizures']},{r['seizure_files_detected']},{r['total_duration_hours']}\n")
    ...
    def run_full_analysis(self, num_thresholds: int = 50):
        ...
        print("Starting Madrid File-Level Threshold Trade-off Analysis...")
        ...
        # Step 4: Create plots
        plot_filename = f"madrid_file_level_threshold_tradeoff_{timestamp}.png"
        self.create_tradeoff_plot(results, plot_filename)
        
        # Step 5: Save results
        results_filename = f"madrid_file_level_threshold_tradeoff_{timestamp}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate threshold trade-off analysis for Madrid file-level metrics"
    )
    parser.add_argument(
        "results_dir", 
    ...
    # Initialize analyzer
    analyzer = MadridThresholdTradeoffAnalyzer(
