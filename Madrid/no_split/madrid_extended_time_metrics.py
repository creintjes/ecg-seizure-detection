#!/usr/bin/env python3
"""
Madrid Extended Time Window Metrics Calculator
...
class MadridExtendedTimeMetrics:
    def __init__(self, results_dir: str, output_dir: str = None, threshold: float = None, 
        ...
        """
        Initialize the extended time window metrics calculator.
    ...
    def save_results(self, results: Dict[str, Any], filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"madrid_extended_time_metrics_{timestamp}"
    ...
    def save_human_readable_report(self, results: Dict[str, Any], filepath: Path):
        ...
        with open(filepath, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MADRID EXTENDED TIME WINDOW SEIZURE DETECTION METRICS REPORT\n")
            f.write("=" * 80 + "\n\n")
            ...
            f.write("OVERALL PERFORMANCE (Extended Time Window):\n")
            f.write("-" * 40 + "\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate file-level sensitivity with extended time windows (5min before, 3min after seizures)"
    )
    parser.add_argument(
        "results_dir", 
    ...
    summary = results['summary_statistics']
    print(f"\n{'='*50}")
    print("EXTENDED TIME WINDOW SUMMARY:")
    print(f"{'='*50}")
