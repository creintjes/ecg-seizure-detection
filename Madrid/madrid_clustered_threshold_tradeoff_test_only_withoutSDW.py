...
class MadridClusteredThresholdTradeoffAnalyzerTestOnly:
    ...
    def calculate_clustered_metrics_for_threshold(self, threshold: float) -> Dict[str, Any]:
        """Calculate clustered metrics for a single threshold using time_600s strategy - TEST SET ONLY."""
        print(f"Processing threshold {threshold:.6f} with time_600s clustering on test set...")
    ...
    def run_full_analysis(self, num_thresholds: int = 50):
        ...
        # Step 4: Create plots
        plot_filename = f"madrid_clustered_time600s_threshold_tradeoff_test_only_{timestamp}.png"
        self.create_tradeoff_plot(results, plot_filename)
        
        # Step 5: Save results
        results_filename = f"madrid_clustered_time600s_threshold_tradeoff_test_only_{timestamp}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate threshold trade-off analysis with time_600s clustering - TEST SET ONLY (sub097-sub125)"
    )
    parser.add_argument(
        "results_dir", 
