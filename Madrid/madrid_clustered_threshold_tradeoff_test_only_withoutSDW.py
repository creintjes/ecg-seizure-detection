...
Tests multiple thresholds with time_600s clustering and plots Sensitivity vs False Alarms per Hour trade-off
...
class MadridClusteredThresholdTradeoffAnalyzerTestOnly:
    ...
    def __init__(self, results_dir: str, output_dir: str = None,
        ...
        self.clustering_time_threshold = 600  # Fixed to time_600s strategy
    ...
    def time_based_clustering(self, anomalies: List[Dict[str, Any]], 
        ...
        Args:
            ...
            time_threshold_seconds: Maximum time distance for clustering (600 seconds)
    ...
    def calculate_clustered_metrics_for_threshold(self, threshold: float, use_training_set: bool = False) -> Dict[str, Any]:
        """
        Calculate clustered metrics for a single threshold using time_600s strategy.
        ...
        print(f"Processing threshold {threshold:.6f} with time_600s clustering on {dataset_name}...")
    ...
    def save_results(self, results: List[Dict[str, Any]], output_filename: str):
        ...
        with open(json_path, 'w') as f:
            json_data = {
                'analysis_metadata': {
                    'analysis_type': 'threshold_tradeoff_clustered_time_600s_test_only',
                    'timestamp': timestamp,
    ...
    def run_full_analysis(self, num_thresholds: int = 50):
        ...
        print(f"{'='*60}")
        training_plot_filename = f"madrid_clustered_time600s_threshold_tradeoff_TRAINING_{timestamp}.png"
        self.create_tradeoff_plot(training_results, training_plot_filename)

        # Step 7: Save training results
        training_results_filename = f"madrid_clustered_time600s_threshold_tradeoff_TRAINING_{timestamp}"
        self.save_results(training_results, training_results_filename)

        # Step 8: Save test results with optimal threshold information
        test_results_filename = f"madrid_clustered_time600s_optimal_thresholds_TEST_{timestamp}"
        ...
        if test_results:
            ...
            for config_name, config_result in best_configs.items():
                ...
                for detection in per_seizure:
                    ...
                    detection['window_type'] = 'no_window'
            ...
            if all_per_seizure_detections:
                ...
                per_seizure_csv_path = self.output_dir / f"madrid_per_seizure_detections_no_window_{timestamp}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate threshold trade-off analysis with time_600s clustering using train/test split. "
