#!/usr/bin/env python3
"""
Madrid Clustered Threshold Trade-off Analysis
...
class MadridClusteredThresholdTradeoffAnalyzer:
    ...
    def calculate_clustered_metrics_for_threshold(self, threshold: float) -> Dict[str, Any]:
        ...
        print(f"Processing threshold {threshold:.6f} with time_180s clustering...")
    ...
    def run_threshold_analysis(self, thresholds: List[float]) -> List[Dict[str, Any]]:
        ...
        print(f"Running clustered analysis for {len(thresholds)} thresholds...")
    ...
    def create_tradeoff_plot(self, results: List[Dict[str, Any]], output_filename: str):
        ...
        ax3.set_title(f'Anomaly Reduction vs Threshold\n(time_{self.clustering_time_threshold}s Clustering)')
        ...
        ax4.set_title(f'Sensitivity vs Anomaly Reduction\n(time_{self.clustering_time_threshold}s Clustering)')
    ...
    def save_results(self, results: List[Dict[str, Any]], output_filename: str):
        ...
        with open(json_path, 'w') as f:
            json.dump({
                'analysis_metadata': {
                    'analysis_type': 'threshold_tradeoff_clustered_time_180s',
                    'timestamp': timestamp,
        ...
        with open(csv_path, 'w') as f:
            f.write("threshold,clustering_strategy,sensitivity,false_alarms_per_hour,total_detections_before_clustering,total_detections_after_clustering,total_true_positives,total_false_positives,total_seizures,detected_seizures,files_processed,total_duration_hours,anomaly_reduction\n")
            for r in results:
                f.write(f"{r['threshold']},{r['clustering_strategy']},{r['sensitivity']},{r['false_alarms_per_hour']},{r['total_detections_before_clustering']},{r['total_detections_after_clustering']},{r['total_true_positives']},{r['total_false_positives']},{r['total_seizures']},{r['detected_seizures']},{r['files_processed']},{r['total_duration_hours']},{r['anomaly_reduction']}\n")
        print(f"CSV results saved to: {csv_path}")
    
    def run_full_analysis(self, num_thresholds: int = 50):
        """Run the complete clustered threshold trade-off analysis."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        print("Starting Madrid Clustered Threshold Trade-off Analysis...")
        ...
        # Step 4: Create plots
        plot_filename = f"madrid_clustered_time180s_threshold_tradeoff_{timestamp}.png"
        self.create_tradeoff_plot(results, plot_filename)
        
        # Step 5: Save results
        results_filename = f"madrid_clustered_time180s_threshold_tradeoff_{timestamp}"
        self.save_results(results, results_filename)
        
        # Step 6: Print summary
        print(f"\n{'='*60}")
        print("CLUSTERED THRESHOLD TRADE-OFF ANALYSIS SUMMARY")
        print(f"{'='*60}")
        print(f"Clustering strategy: time_{self.clustering_time_threshold}s")
        print(f"Extended window: -{self.pre_seizure_seconds/60:.1f} min to +{self.post_seizure_seconds/60:.1f} min")
        ...
        if results:
            ...
            print(f"\nBest Sensitivity: {best_sens['sensitivity']:.4f}")
            ...
            print(f"\nBest Anomaly Reduction: {best_reduction['anomaly_reduction']:.4f} ({best_reduction['anomaly_reduction']*100:.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate threshold trade-off analysis with time_180s clustering for Madrid extended time window metrics"
    )
    parser.add_argument(
        "results_dir", 
    ...
    # Initialize analyzer
    analyzer = MadridClusteredThresholdTradeoffAnalyzer(
