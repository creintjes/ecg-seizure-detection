#!/usr/bin/env python3
"""
Madrid File-Level Metrics Calculator - TEST SET ONLY
...
class MadridFileLevelMetricsTestOnly:
    def __init__(self, results_dir: str, output_dir: str = None, threshold: float = None):
        """
        Initialize the file-level metrics calculator for test set only.
        ...
        self.output_dir = Path(output_dir) if output_dir else self.results_dir / "test_only_file_metrics"
    ...
    def save_results(self, results: Dict[str, Any], filename: str = None):
        ...
        if filename is None:
            ...
            filename = f"madrid_file_level_metrics_test_only_{timestamp}"
    ...
    def save_human_readable_report(self, results: Dict[str, Any], filepath: Path):
        ...
        with open(filepath, 'w') as f:
            f.write("=" * 80 + "\n")
            f.write("MADRID FILE-LEVEL SEIZURE DETECTION METRICS REPORT - TEST SET ONLY\n")
            f.write("=" * 80 + "\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Calculate file-level sensitivity and false alarms per hour - TEST SET ONLY (sub097-sub125)"
    )
    parser.add_argument(
        "results_dir", 
    ...
    parser.add_argument(
        "-o", "--output-dir", 
        help="Output directory for metrics results (default: results_dir/test_only_file_metrics)"
    )
    parser.add_argument(
        "-t", "--threshold", 
        type=float,
    ...
    calculator = MadridFileLevelMetricsTestOnly(
    ...
    summary = results['summary_statistics']
    print(f"\n{'='*50}")
    print("FILE-LEVEL SUMMARY - TEST SET ONLY:")
    print(f"{'='*50}")
